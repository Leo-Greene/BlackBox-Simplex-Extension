...
% plot_integrated_trajs.m
% Targeted plotting script for Step 4 Integrated Evaluation results.
% It automatically finds the latest manifest.mat in traj/step4_integrate.
...
output_root = fullfile(PROJECT_ROOT, 'traj', 'step4_integrate');
manifest_mat = '';

% --- 自动寻找最新的结果文件夹 (基于文件夹名称中的时间戳) ---
if isempty(manifest_mat)
    % 获取所有符合日期格式 (20xx-xx-xx_xxxxxx) 的文件夹
    d = dir(output_root);
    d = d([d.isdir]);
    names = {d.name};
    ...
    if ~any(valid_idx)
        error('未找到符合时间戳命名的 Step 4 结果文件夹于: %s', output_root);
    end
...
if use_parallel
    ...
    parfor i = 1:num_cases
        ...
        % Each worker creates its own invisible figure
        ...
        % Title & Style
        is_collision = false;
        if isfield(manifest, 'has_collision'), is_collision = manifest(i).has_collision; end
        
        if is_collision
            title(['[COLLISION] Integrate Case ', case_id_str, ' (Workers: ', num2str(manifest(i).num_collisions), ')'], 'Color', 'r', 'FontSize', 14);
            img_name_suffix = '_COLLISION.png';
        else
            title(['[SAFE] Integrate Case ', case_id_str], 'Color', [0 0.5 0], 'FontSize', 14);
            img_name_suffix = '.png';
        end
        
        [case_dir, ~, ~] = fileparts(mat_file);
        img_name = fullfile(case_dir, ['plot_integrate_case_', case_id_str, img_name_suffix]);
